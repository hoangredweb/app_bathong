...
* @package	Managementcom
*/
class ManagementcomCkHelperForm
{
	/**
	...
	*/
	public static function getElementGroups($element)
	{
		...
		$model = ManagementcomHelper::componentModel((string)$element['model'], false);
		...
		foreach ($element->children() as $option)
		{
			...
			if ($model)
			{
				...
				$group->model = (strpos($relation->model, '.')?'':'managementcom.') . $relation->model;

				$model = ManagementcomHelper::componentModel($relation->model, false);
			}
			...
			// Label Key
			$group->alias = ManagementcomClassModelOrm::tableFieldAlias($group->on, false);
	...
	*/
	public static function getModel($field, $item = false)
	{
		...
		return ManagementcomHelper::componentModel($model, $item);
	}
...
if (!class_exists('ManagementcomHelperForm')){ class ManagementcomHelperForm extends ManagementcomCkHelperForm{} }
