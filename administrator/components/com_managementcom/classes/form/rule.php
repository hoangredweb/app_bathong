...
/**
* Form validator rule for Managementcom.
*
* @package	Managementcom
* @subpackage	Form
*/
class ManagementcomCkClassFormRule extends JFormRule
{
	/**
	...
	*/
	public function getJsonRule($field)
	{
		...
		*/
		...
		$json = ManagementcomHelperHtmlValidator::jsonFromArray($values);
		return "{" . LN . $json . LN . "}";
	}
	...
	*/
	public static function testDefaults(&$element, $value, $group = null, &$input = null, &$form = null)
	{
		...
		if ($unique)
		{
			...
			if ($duplicate)
			{
				JError::raiseWarning(1201, JText::sprintf("MANAGEMENTCOM_MODEL_DUPLICATE_ENTRY_FOR", JText::_($element['label'])));
				return false;
			}
		}
		...
		if (!empty($uniqueGroups))
		{
			...
			foreach($uniqueGroups as $alias => $uniqueGroup)
			{
				...
				if ($duplicate)
				{
					JError::raiseWarning(1201, JText::sprintf("MANAGEMENTCOM_MODEL_DUPLICATE_ENTRY_FOR", implode (', ', $fieldLabels)));
					return false;
				}
			}
		}
...
if (!class_exists('ManagementcomClassFormRule')){ class ManagementcomClassFormRule extends ManagementcomCkClassFormRule{} }
