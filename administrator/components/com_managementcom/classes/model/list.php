...
* @subpackage	Classes
*/
class ManagementcomCkClassModelList extends JModelList
{
	/**
	* Data array
	*
	...
	*/
	...
	{
		$model = CkJModel::getInstance($this->view_item, 'ManagementcomModel');
	...
	*/
	public function getItems()
	{
		try
		{
			...
			{
				if (!$this->canAdmin())
					$error = JText::_('MANAGEMENTCOM_ERROR_INVALID_QUERY');
	...
	*/
	...
	{
		if ($item)
			return CkJModel::getInstance($this->view_item, 'ManagementcomModel');
	...
	*/
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$globalParams = JComponentHelper::getParams('com_managementcom', true);
		...
		// FILTERS
			foreach($this->filter_vars as $var => $varType)
			{
				...
				{
					$date = ManagementcomHelperDates::timeFromFormat($value, $matches[1]);
					if ($date)
					{
	...
	*/
	public function updateXref($field, $values, $on, $id)
	{
		...
		$model = CkJModel::getInstance($this->getName(), 'ManagementcomModel');
		...
		else
		{
			...
			{
				$query = $db->getQuery(true);
				$query->update('#__managementcom_' . $this->getName())
...
if (!class_exists('ManagementcomClassModelList')){ class ManagementcomClassModelList extends ManagementcomCkClassModelList{} }
