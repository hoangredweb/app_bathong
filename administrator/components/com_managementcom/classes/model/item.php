...
* @subpackage	Classes
*/
class ManagementcomCkClassModelItem extends JModelAdmin
{
	/**
	* Data array
	*
	...
	* @var string
	*/
	protected static $extension = 'managementcom';
	...
	*/
	public function __construct($config = array())
	{
		...
		// Load the ORM system
		$this->orm = ManagementcomClassModelOrm::getInstance($this);
	...
	*/
	public function canCheck($record)
	{
		...
		{			
			$this->setError(JText::_("MANAGEMENTCOM_TASK_RESULT_THE_USER_CHECKING_OUT_DOES_NOT_MATCH_THE_USER_WHO_CHECKED_OUT_THE_ITEM"));
			return false;			
		}
	...
	*/
	public function deletefiles($pks, $fileFields)
	{
		...
		$config	= JComponentHelper::getParams( 'com_managementcom' );
		...
		foreach($fileFields as $fieldName => $op)
		{
			...
			foreach($files as $fileObj)
			{
				...
				if (!ManagementcomHelperFile::removeFile($imagePath, $op))
					$errors = true;
			}
		}
	...
	*/
	public static function fkTable($fieldname)
	{
		...
		switch($fieldname)
		{
			...
			case 'customer_id': return $tbl.$com. 'customers';
			case 'product_id': return $tbl.$com. 'products';	
		}
	}
	...
	*/
	...
	{
		$model = CkJModel::getInstance(ucfirst($table), 'ManagementcomModel');
	...
	*/
	public function getForm($data = array(), $loadData = true, $control = 'jform')
	{
		...
		$form->addRulePath(JPATH_ADMIN_MANAGEMENTCOM . '/models/rules');
	...
	*/
	...
	{
		$modelList = CkJModel::getInstance($this->getNameList(), 'ManagementcomModel');
		if ($modelList)
	...
	*/
	protected function loadForm($name, $source = null, $options = array(), $clear = false, $xpath = false)
	{
		...
		// Get the form.
		JForm::addFormPath(JPATH_MANAGEMENTCOM . '/models/forms');
		JForm::addFieldPath(JPATH_MANAGEMENTCOM . '/models/fields');
	...
	*/
	public function loadRelation($name, &$item, $orm = array())
	{
		...
		$model = ManagementcomClassModelOrm::getModel($relation->foreignModelClass);
		if (!$model)
			return;
	...
	*/
	public function loadXref(&$item, $objectField, $xrefTable, $on, $key, $states = array(), $context = 'object.default')
	{
		...
		{
			$model = CkJModel::getInstance($xrefTable, 'managementcomModel');
	...
	*/
	protected function populateState()
	{
		...
		if (defined('JDEBUG'))
			$_SESSION["Managementcom"]["Model"][$this->getName()]["State"] = $this->state;
	...
	*/
	...
	{
		$baseFolder = JPATH_MANAGEMENTCOM . '/fork/models/forms';
	...
	*/
	...
	{
		return ManagementcomClassModelOrm::queryDump($query, $output);
	}
	...
	*/
	public function toggle($fieldName, $pk = null, $max = 1)
	{
		...
		if (!$result)
		{
			JError::raiseWarning(1106, JText::sprintf("MANAGEMENTCOM_MODEL_IMPOSSIBLE_TO_TOGGLE", $fieldName));
			return false;
		}
			...
			switch($field->type)
			{
				...
				case 'ckcalendar':
					...
					if ($value && (string)$field->format)
					{
						...
						if (!is_numeric($value) && !ManagementcomHelperDates::isNull((string)$value))
						{
							$time = ManagementcomHelperDates::getSqlDate($value, array($field->format));

							if ($time === null){
								JError::raiseWarning(1203, JText::sprintf('MANAGEMENTCOM_VALIDATOR_WRONG_DATETIME_FORMAT_FOR_PLEASE_RETRY', $field->label));
								$valid = false;
							}
							else
...
if (!class_exists('ManagementcomClassModelItem')){ class ManagementcomClassModelItem extends ManagementcomCkClassModelItem{} }
