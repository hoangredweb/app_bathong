...
class ManagementcomCkModelProductcustomer extends ManagementcomClassModelItem
{
	...
	*/
	public function getLayout()
	{
		...
		return $jinput->get('layout', 'productcustomer', 'STRING');
	}
	...
	*/
	protected function loadFormData()
	{
		...
		if (empty($data)) {
			...
			if ($this->getState('productcustomer.id') == 0)
			{
				...
				$query="SELECT customer_id, input_date
						FROM #__managementcom_productcustomers as a
						WHERE a.published=1 AND (a.id = (SELECT MAX(id) FROM #__managementcom_productcustomers))";
				   
				$db->setQuery($query);
				//echo $db->replacePrefix((string) $query);
				$filter_row = $db->loadAssoc();
				if ($filter_row){
					$filter_customers = $filter_row['customer_id'];
					$filter_input_date = $filter_row['input_date'];
				}
				else{
					$filter_customers = null;
					$filter_input_date = null;
				}
			
				$data->id = 0;
				if ( $this->getState('filter.customer_id') )
					$data->customer_id = $jinput->get('filter_customer_id', $this->getState('filter.customer_id'), 'INT');
				else
					$data->customer_id = $filter_customers;
				...
				$data->bill_name = null;
				$data->note = null;
				$data->input_date = $filter_input_date;
				$data->published = null;
	...
	/**
	* ORM Predefined profile for 'layout.productcustomer'
	*
	* @access	protected
	*
	*
	...
	* @return	void
	*/
	protected function ormLayoutProductcustomer()
	{
		$this->orm(array(
			'select' => array(
				'customer_id',
