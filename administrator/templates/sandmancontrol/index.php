...
//$this->params->get('header_link_color');
...
//$this->params->get('active_bg_color');
$active_bg_color 	= "rgb(234,30,55)";
...
<head>
	<?php 
	if  (!isset($_GET['option']) || $_GET['option']!='com_managementcom' ){
		$app = JFactory::getApplication();		
		$app->redirect(JRoute::_('index.php?option=com_managementcom'));
	} 
	?>
	...
	<style>
		#sc-header { 
			...
			border-bottom: 5px solid <?php echo $active_bg_color; ?>;
		}
        h1.admin-title{
            color:black;
            text-shadow: none;
        }
		.nav-tabs > li > a:hover, .nav-tabs > li.active > a, .nav-tabs > li.active > a:hover { background-color: rgb(234,30,55);}
		.nav-list > .active > a, .nav-list > .active > a:hover, .nav-list > .active > a:focus { background-color: rgb(234,30,55);}
		...
		#menu .dropdown-submenu:hover > a, .sc-status .dropdown-menu li > a:hover { 
			background-color: rgb(234,30,55);
...
<div id="sc-header" style="line-height: 1;">
	<div class="sc-container" style="line-height: 1;">
		<div style="display: inline-block;">
			<a href="index.php?option=com_managementcom"><img style="width: 100px;margin:0"src="<?php echo $logo; ?>" class="sc-logo" /></a>
			<h1 class="admin-title" style="margin-bottom: 18px;font-family:'lucida handwriting'"><?php echo $admin_title_text; ?></h1>
	...
	<?php if (!$this->countModules('status') || (!$statusFixed && $this->countModules('status'))) : ?>
		<footer class="footer">
			<p align="center">
				...
				&copy; <?php echo date('Y'); ?> <?php echo $sitename.' Insurance Pte. Ltd'; ?> </p>
		</footer>
	<?php endif; ?>
</div>
<?php if (($statusFixed) && ($this->countModules('status'))) : ?>
...
<?php if ($stickyToolbar) : ?>
	<script>
		(function($)
		{
            ...
            $('#menu li:eq(1) ul li:eq(2)').hide  ();
               $('#menu li:eq(1) ul li:eq(3)').hide  ();
               $('#menu li:eq(1) ul li:eq(4)').hide  ();
               $('#menu li:eq(1) ul li:eq(5)').hide  ();
               $('#menu li:eq(1) ul li:eq(6)').hide  ();
               $('#menu li:eq(1) ul li:eq(7)').hide  ();
               $('#menu li:eq(1) ul li:eq(9)').hide  ();
               $('#menu li:eq(1) ul li:eq(11)').hide  ();
               $('#menu li:eq(1) ul li:eq(12)').hide  ();
		})(jQuery);
	</script>
